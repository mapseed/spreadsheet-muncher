...
IMPORT_CSV_FILE = 'Tracy-urbanwaters-working_v7_precall.csv'
# 'Tracy-urbanwaters-working_v7_precall.ods_Master.csv'
...
def crunchCSV(inFileName, outFileName):
    ...
    with open(inFileName, 'r') as csvinput,\
        ...
        # Control flow boolean to keep track of whether we've reached
        # end of input file
        ...
        # Various counters to help with debugging the while loop that
        # reads the file
        ...
        try:
            ...
            # 2nd row is first row of data
            currentRow = reader.next()
            # store the ID for future use
            currentID = currentRow['Location ID']
            # Start counting rows of data
            row_count += 1
        ...
        while(not end_of_csvinput):
            # Create a list to store the current row and the matching rows
            # to be merged
            rows = [currentRow]

            # Try reading the next row. As long as a next row exists and
            # its ID matches that of the current row, continue adding
            # rows to the list.
            try:
                nextRow = reader.next()
                ...
                # currentRow contains the last ID in the file
                end_of_csvinput = True
            ...
            # newRow should be a dictionary
            newRow = mergeRows(rows, newRowNum)
            ...
            # print("row count: %i\n" % row_count)
            # # print("Number of distinct decimal ID's = %i" %
            # # sum(row_group_counter.values()))
            # print("Number of distinct ID's = %i" %
            # sum(row_group_counter.values()))
            # print("Computed number of rows = %i\n" % sum([ k*v for (k,v)
            # in row_group_counter.iteritems()]))
        ...
        print('Last row number+1 - first row number:', newRowNum -
              FIRST_NUMBER)
...
def mergeRows(rows, rowNumber):
    """
    Args:
        rows (list) = list of rows (each a dictionary)
        rowNumber (int) = number to assign the new row
    ...
    print("Sources for Location ID %s:" %
          newRow['Location ID'], newRow['Source'])
    ...
    # elif (numRows == 2):
        ...
        # Probably we should test whether we ended up using information
        # from both sources
        # newRow['Source'] = ???
        newRow['URL'] = selectContent('URL', rows,
                                      test_val=DEFAULT_URL,
...
def selectContent(header, rows, test='dominant',
    ...
    if test == 'dominant':
        ...
        # If no dominant field is found, use the default value
        empty_is_ok = True
